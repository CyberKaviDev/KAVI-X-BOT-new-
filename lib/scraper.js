...
async function tiktok(videoUrl) {
    try {
        const response = await axios.post('https://api.musicallydown.live/get-data', 
            { url: videoUrl },
            {
                headers: {
                    'Content-Type': 'application/json',
                    'Origin': 'https://ssstok.net',
                    'Referer': 'https://ssstok.net/',
        ...
        const data = {
            mp3_url: response.data.mp3_url,
            no_watermark_url: response.data.no_watermark_url,
            no_watermark_url_hd: response.data.no_watermark_url_hd,
            thumbnail_url: response.data.thumbnail_url,
            title: response.data.title,
            watermark_url: response.data.watermark_url
        };
...
async function isValidTikTokUrl(url) {
    // Regular expression for TikTok URL validation
    const regex = /^https:\/\/(www\.)?tiktok\.com\/.*|^https:\/\/vt\.tiktok\.com\/.*$/;
...
async function scrapeCsrfToken() {
    const baseURL = 'https://fbload.online';
  ...
  async function facebook(videoUrl) {
    ...
    try {
      ...
      // Check if medias exist and is an array
      if (!response.data.medias || !Array.isArray(response.data.medias)) {
        return { error: true, message: "No media data available." };
      }
  
      // Create formatted output with HD, SD, and MP3
      const mediaFormats = response.data.medias.reduce((result, media) => {
        if (media.quality === 'HD') {
          result.HD = {
            url: media.url,
...
module.exports = { facebook, tiktokV2, ffstalk, mlstalk, npmstalk, mlstalk, githubstalk, pinterest, wallpaper, wikimedia, porno, hentai, quotesAnime, igstalk, aiovideodl, umma, ringtone, styletext, tiktok, isValidTikTokUrl }